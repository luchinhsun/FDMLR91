...
__device__ void comp_ib(double *d_V, double *d_it, int k, int j) {

        d_it[k] += 0.03921*(d_V[k+nx+2+1+2*j] + 59.87);

}

__device__ void comp_ikp(double *d_V, double *d_it, int k, int j) {
...
__device__ void comp_ik1(double *d_V, double *d_it, int k, int j) {
...
__device__ void comp_ik(double *d_V, double *d_X, double *d_it, int k, int j){
...
__device__ void comp_ical(double *d_V, double *d_d, double *d_f, double *d_cai, double *d_it, int k, int j, int i){
...
__device__ void comp_ina(double *d_V, double *d_m, double *d_h, double *d_jj, double *d_it, int k, int j) {
	//int id = k+nx+2+1+2*j;
	d_it[k] = 0.0;
...
__global__ void comp_it(double *d_V, double *d_m, double *d_h, double *d_jj,
                                 double *d_d, double *d_f, double *d_cai,
                                         double *d_X, double *d_it) {
	...
	comp_ina(d_V, d_m, d_h, d_jj, d_it, k, j);
        comp_ical(d_V, d_d, d_f, d_cai, d_it, k, j, i);
	comp_ik(d_V, d_X, d_it, k, j);
	comp_ik1(d_V, d_it, k, j);
	comp_ikp(d_V, d_it, k, j);
	comp_ib(d_V, d_it, k, j);
...
void gpu(){
	...
	boundary<<<bpg, tpb>>>(d_V);
        bpg = (nx*ny+tpb-1)/tpb;
	comp_it<<<bpg, tpb>>>(d_V, d_m, d_h, d_jj, d_d, d_f, d_cai, d_X, d_it);
	//comp_ina<<<bpg, tpb>>>(d_V, d_m, d_h, d_jj, d_it);
	//comp_ical<<<bpg, tpb>>>(d_V, d_d, d_f, d_cai, d_it);
	//comp_ik<<<bpg, tpb>>>(d_V, d_X, d_it);
