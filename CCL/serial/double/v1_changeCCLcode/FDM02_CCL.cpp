...
int main(int argc, char* argv[])
{
	...
	int nstep = 4 / dt_max; // snapshot interval to save data files
	...
	for (ncount = 1; ncount <= 160 / dt_max; ncount++){//simulation time is 160ms
		for (i = 1; i < nx + 1; i++){
			...
			for (i = 1; i < nx + 1; i++){
				for (j = 1; j < ny + 1; j++){
					...
					while(dt_sum<dt_max){
						...
						dVdt[i][j] = -it[i][j] + (-st)*(ncount >= 1 && ncount <= stimtime && j >= 1 && j <= 5);
