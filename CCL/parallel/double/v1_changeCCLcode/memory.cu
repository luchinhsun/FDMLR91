...
void Allocate(){
	...
	*/
	...
	cudaMalloc((void**)&d_D1V, size);
	cudaMalloc((void**)&d_D2V, size);
}
...
void free(){
	...
	cudaFree(d_isi);cudaFree(d_dVdt);cudaFree(d_D1V);
	//cudaFree(dcai);
	cudaFree(d_D2V);
	/*
...
/*
        ...
        Error = cudaMemcpy(h_Vnew, d_t, size, cudaMemcpyDeviceToHost);
        if (Error != cudaSuccess)
