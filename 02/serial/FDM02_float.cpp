...
/* Time Step */
float dt_max = 0.04; // Time step (ms)
float dt_min = 0.001;
float dt;
float t; // Time (ms)
...
float dVdt[nx + 2][nx + 2]; // first order derivatives of Voltage (mv)
...
float m0[nx + 1][nx + 1], h0[nx + 1][nx + 1], jj0[nx + 1][nx + 1];
float d0[nx + 1][nx + 1], f0[nx + 1][nx + 1];
float X0[nx + 1][nx + 1];

//performance compared
float Vmax, V_left = 0, V_right = 0, left_peak, right_peak, conduction_t = 0;
float APD90; // Time of 90% Repolarization 
float Vold, v_onset;
