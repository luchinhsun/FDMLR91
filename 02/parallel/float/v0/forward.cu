...
__device__ void comp_it(float *d_V, float *d_m, float *d_h, float *d_jj,
                                 float *d_d, float *d_f, float *d_cai, float *dcai,
                                         float *d_X, float *d_it, float *d_m0, float *d_h0, float *d_jj0,
                                                float *d_d0, float *d_f0, float *d_X0, int I, int i, int k, float *d_t) {
	//int id = k+nx+2+1+2*j;
	d_it[k] = 0.0;

	//comp_ina
	float gna = 23;
        float ena = ((R*temp) / frdy)*log(nao / nai);

        float am = 0.32*(d_V[k+nx+2+1+2*i] + 47.13) / (1 - exp(-0.1*(d_V[k+nx+2+1+2*i] + 47.13)));
        float bm = 0.08*exp(-d_V[k+nx+2+1+2*i] / 11);
	float ah, bh, aj ,bj;
	...
	__shared__ float isi[tpb];
        ...
        float ad = 50 * 0.095*exp(-0.01*(d_V[k+nx+2+1+2*i] - 5)) / (1 + exp(-0.072*(d_V[k+nx+2+1+2*i] - 5)));
        float bd = 50 * 0.07*exp(-0.017*(d_V[k+nx+2+1+2*i] + 44)) / (1 + exp(0.05*(d_V[k+nx+2+1+2*i] + 44)));
        float af = 50 * 0.012*exp(-0.008*(d_V[k+nx+2+1+2*i] + 28)) / (1 + exp(0.15*(d_V[k+nx+2+1+2*i] + 28)));
        float bf = 50 * 0.0065*exp(-0.02*(d_V[k+nx+2+1+2*i] + 30)) / (1 + exp(-0.2*(d_V[k+nx+2+1+2*i] + 30)));
	...
	//comp_ik
        ...
        float ek = ((R*temp) / frdy)*log(ko / ki);
        //float prnak = 0.01833;
        //ek = ((R*temp) / frdy)*log((ko + prnak*nao) / (ki + prnak*nai));

        float ax = 50 * 0.0005*exp(0.083*(d_V[k+nx+2+1+2*i] + 50)) / (1 + exp(0.057*(d_V[k+nx+2+1+2*i] + 50)));
        float bx = 50 * 0.0013*exp(-0.06*(d_V[k+nx+2+1+2*i] + 20)) / (1 + exp(-0.04*(d_V[k+nx+2+1+2*i] + 20)));
	...
	float Xi;
        if (d_V[k+nx+2+1+2*i] > -100) {
                Xi = 2.837*(exp(0.04*(d_V[k+nx+2+1+2*i] + 77)) - 1)/
			((d_V[k+nx+2+1+2*i] + 77 + 1e-15)*exp(0.04*(d_V[k+nx+2+1+2*i] + 35)));
        }
        else {
                Xi = 1;
        }
	...
	//comp_ik1
        ...
        float ek1 = ((R*temp) / frdy)*log(ko / ki);

        float ak1 = 1.02 / (1 + exp(0.2385*(d_V[k+nx+2+1+2*i] - ek1 - 59.215)));
        float bk1 = (0.49124*exp(0.08032*(d_V[k+nx+2+1+2*i] - ek1 + 5.476))+
			exp(0.06175*(d_V[k+nx+2+1+2*i] - ek1 - 594.31)))
                	/(1 + exp(-0.5143*(d_V[k+nx+2+1+2*i] - ek1 + 4.753)));
        float K1ss = ak1 / (ak1 + bk1);

        d_it[k] += gk1*K1ss*(d_V[k+nx+2+1+2*i] - ek1);
	//comp_ikp
        float gkp = 0.0183;
        float ekp = ((R*temp) / frdy)*log(ko / ki);

        float kp = 1 / (1 + exp((7.488 - d_V[k+nx+2+1+2*i]) / 5.98));
...
__global__ void comp_dVdt(float *d_V, float *d_m, float *d_h, float *d_jj,
                                 float *d_d, float *d_f, float *d_cai, float *dcai,
                                         float *d_X, float *d_it, float *d_m0, float *d_h0, float *d_jj0,
                                                float *d_d0, float *d_f0, float *d_X0, float *d_dVdt, float *d_t){
...
__global__ void comp_ODE_stim(float *d_V, float *d_m, float *d_h, float *d_jj,
                                 float *d_d, float *d_f, float *d_cai, float *dcai,
                                         float *d_X, float *d_it, float *d_m0, float *d_h0, float *d_jj0,
                                                float *d_d0, float *d_f0, float *d_X0, float *d_dVdt, float *d_t){
...
__global__ void comp_ODE(float *d_V, float *d_m, float *d_h, float *d_jj,
                                 float *d_d, float *d_f, float *d_cai, float *dcai,
                                         float *d_X, float *d_it, float *d_m0, float *d_h0, float *d_jj0,
                                                float *d_d0, float *d_f0, float *d_X0, float *d_dVdt, float *d_t, int num){
...
void ODE(){
	int bpg;
        bpg = (nx*ny+tpb-1)/tpb;
        comp_ODE<<<bpg, tpb>>>(d_V, d_m, d_h, d_jj, d_d, d_f, d_cai, dcai, d_X, d_it, d_m0, d_h0, d_jj0, d_d0, d_f0, d_X0, d_dVdt, d_t, 0);
}
